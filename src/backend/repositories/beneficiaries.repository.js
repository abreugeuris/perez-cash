import { supabase } from '@/lib/supabaseClient';
export const beneficiariesRepository = {
  async getAll() {
    const { data, error } = await supabase.rpc('get_beneficiaries')
    if (error) throw error
    return data ?? []
  },

  async create(payload) {
    const { data, error } = await supabase.rpc('create_beneficiary', {
      p_name:           payload.name,
      p_phone:          payload.phone,
      p_id_document:    payload.idDocument ?? null,
      p_country:        payload.country ?? null,
      p_payment_method: payload.paymentMethod ?? null,
      p_bank_account:   payload.bankAccount ?? null,
      p_bank:           payload.bank ?? null,
      p_notes:          payload.notes ?? null,
    })
    if (error) throw error
    return data?.[0] ?? null
  },

  async update(id, payload) {
    const { data, error } = await supabase.rpc('update_beneficiary', {
      p_id:             id,
      p_name:           payload.name,
      p_phone:          payload.phone,
      p_id_document:    payload.idDocument ?? null,
      p_country:        payload.country ?? null,
      p_payment_method: payload.paymentMethod ?? null,
      p_bank_account:   payload.bankAccount ?? null,
      p_bank:           payload.bank ?? null,
      p_notes:          payload.notes ?? null,
    })
    if (error) throw error
    return data?.[0] ?? null
  },

  async delete(id) {
    const { error } = await supabase.rpc('delete_beneficiary', { p_id: id })
    if (error) throw error
  },
}